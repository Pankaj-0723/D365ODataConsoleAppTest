...
namespace ODataConsoleApp
{
    class SimpleCRUDExamples
    {
        public static void SimpleCRUD(Resources d365)
        {
            ...
            var customerData = d365.Customers.Where(x => x.DataAreaId == "usmf").ToList();// && x.PartyNumber == "000000838" && x.CustomerGroupId== "90").First();
            Console.WriteLine(JsonConvert.SerializeObject(customerData));
            // Below line will read CustomerGroup data from the service.
            var assetMajorType = d365.CustomerGroups.Where(x => x.DataAreaId == company).ToList();
            ...
            var salesInvoiceHeaderData = d365.SalesInvoiceHeaders.Where(x => x.DataAreaId == "usmf").ToList();// && x.InvoiceNumber == "CIV-000001").First();
            ...
            var salesInvoiceLineData = d365.SalesInvoiceLines.Where(x => x.DataAreaId == "usmf").ToList();// && x.InvoiceNumber == "CIV-000001").First();
